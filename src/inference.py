...
def inference(input_tensor, train=True, regularizer=None):
    ...
    with tf.variable_scope("conv1"):
        weights = tf.get_variable('weight', [11, 11, 3, 96], initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    lrn1 = tf.nn.lrn(relu1, depth_radius=5, biases=2, alpha=0.0001, beta=0.75)
    pool1 = tf.nn.max_pool(lrn1, [1, 3, 3, 1], [1, 2, 2, 1], padding="VALID")

    with tf.variable_scope("conv2"):
        weights = tf.get_variable('weight', [5, 5, 96, 256], initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    lrn2 = tf.nn.lrn(relu2, depth_radius=5, biases=2, alpha=0.0001, beta=0.75)
    pool2 = tf.nn.max_pool(lrn2, [1, 3, 3, 1], [1, 2, 2, 1], padding="VALID")

    with tf.variable_scope("conv3"):
        weights = tf.get_variable('weight', [3, 3, 256, 384], initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope("conv4"):
        weights = tf.get_variable('weight', [3, 3, 384, 384], initializer=tf.truncated_normal_initializer(stddev=0.1))
        biases = tf.get_variable('biases', [384], initializer=tf.constant_initializer(0.0))
    ...
    with tf.variable_scope("conv5"):
        weights = tf.get_variable('weight', [3, 3, 384, 256], initializer=tf.truncated_normal_initializer(stddev=0.1))
        biases = tf.get_variable('biases', [256], initializer=tf.constant_initializer(0.0))
    ...
    with tf.variable_scope("fc6"):
        weights = tf.get_variable('weight', [9216, 4096], initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope("fc7"):
        weights = tf.get_variable('weight', [4096, 4096], initializer=tf.truncated_normal_initializer(stddev=0.1))
        biases = tf.get_variable('biases', [4096], initializer=tf.constant_initializer(0.0))
    ...
    with tf.variable_scope("fc8"):
        weights = tf.get_variable('weight', [4096, NUM_CLASS], initializer=tf.truncated_normal_initializer(stddev=0.1))
...
def load_weights_biases(sess, mode_pathname):
    weights_dict = np.load(mode_pathname, encoding='bytes').item()

    for op_name in weights_dict:
        with tf.variable_scope(op_name, reuse=True):
            trainable = True
            if op_name in TRAIN_LAYERS:
                trainable = False

            weights = tf.get_variable("weights", trainable=trainable)
            sess.run(weights.assign(weights_dict[op_name]["weights"]))

            biases = tf.get_variable("biases", trainable=False)
            sess.run(biases.assign(weights_dict[op_name]["biases"]))
